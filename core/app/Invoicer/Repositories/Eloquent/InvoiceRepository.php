...
class InvoiceRepository extends BaseRepository implements InvoiceInterface{
    ...
    public function invoiceTotals($uuid){
        ...
        $totals['amountDue']        = float_subtraction($totals['grandTotal'],  $paid);
       return $totals;
    }
    /**
     * @return int|mixed
    ...
    public function ajaxSearch(){
        $term = request('data')['q'];
        $invoices = $this->where('invoice_no', '%'.$term.'%','LIKE')->get();
        $results = array();
        foreach($invoices as $invoice){
            $record = [
                'id'     => $invoice->uuid,
                'text'   => $invoice->invoice_no.' ('.$invoice->totals['grandTotal'].') - '.strtoupper($invoice->client->name)
            ];
    ...
    public function changeStatus($id){
        ...
        $grandTotal = format_amount($invoice->totals['grandTotal']);
