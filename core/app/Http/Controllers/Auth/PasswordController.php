        ...
        if ($mail_setting) {
            try {
                $response = $this->passwords->sendResetLink($request->only('email'), function($message) use($settings,$mail_setting) {
                    $message->from($mail_setting ? $mail_setting->from_email : 'noreply@gestaousuarios.com', $mail_setting ? $mail_setting->from_name : 'gestaousuarios');
                    $message->sender($mail_setting ? $mail_setting->from_email : 'noreply@gestaousuarios.com', $mail_setting ? $mail_setting->from_name : 'gestaousuarios');
                    $message->subject(trans('app.password_reminder'));
                });
                switch ($response){
            ...
            Flash::error(trans('app.email_settings_error'));
            return redirect()->back();
        }
    }
}
