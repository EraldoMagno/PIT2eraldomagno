    ...
    <section class="content">
        <div class="card border-top-primary">
            <div class="card-header">
                <a href="{{ route('estimates.index') }}" class="btn btn-sm btn-outline-info"> <i class="fa fa-arrow-circle-left"></i> @lang('app.back')</a>
                @if(hasPermission('send_estimate'))
                    <a href="{{route('estimate_send_modal',$estimate->uuid)}}" data-toggle="ajax-modal" class="btn btn-success btn-sm pull-right" style="margin-left: 5px"> <i class="fa fa-send"></i> @lang('app.send')</a>
                @endif
                    <a href="{{ url('estimates/pdf', $estimate->uuid) }}" class="btn btn-primary btn-sm pull-right" style="margin-left: 5px"> <i class="fa fa-download"></i> @lang('app.download')</a>
                @if(hasPermission('edit_estimate'))
                    <a href="{{ route('estimates.edit', $estimate->uuid) }}" class="btn btn-warning btn-sm pull-right" > <i class="fa fa-pencil"></i> @lang('app.edit')</a>
                @endif
            </div>
            <div class="card-body">
                <div class="row">
