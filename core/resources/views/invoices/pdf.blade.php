...
<div class="container">
    ...
    <div class="text-right" style="width:300px;height:130px;float:right;">
        <div class="text-right text-uppercase"> <h2>{{trans('app.invoice')}}</h2></div>
        <table style="width: 100%">
            ...
            <tr>
                <td class="text-right">{{trans('app.due_date')}}</td>
                <td class="text-right">{{ format_date($invoice->due_date) }}</td>
            </tr>
            @if($settings && $settings->vat != '')
                <tr>
                    <td class="text-right">{{trans('app.vat_number')}}</td>
    ...
    <div class="col-md-12 form-group">
        ...
        <div class="to_address">
            <h4 class="invoice_title">{{trans('app.billing_to')}} </h4><hr class="separator"/>
            <h4 class="text-uppercase">{{ $invoice->client->name }}</h4>
            ...
            @if($invoice->client->phone != '')
                {{trans('app.phone')}}: {{ $invoice->client->phone }}<br/>
            @endif
            @if($invoice->client->email != '')
                {{trans('app.email')}}: {{ $invoice->client->email }}.
            @endif
        </div>
    </div>
    <div style="clear: both"></div>
    ...
    @if($invoice->totals['amountDue'] > 0 && $invoiceSettings && $invoiceSettings->show_pay_button)
        <div class="col-md-12" style="margin-top:20px;text-align: right">
          <a target="_blank" href="{{url('clientarea/cinvoices')}}" style="float:right" class="btn-success">@lang('app.pay_invoice')</a>
        </div>
    @endif
</div>
</body></html>
