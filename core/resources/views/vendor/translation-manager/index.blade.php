@extends('app')
@push('styles')
{{ Html::style('assets/plugins/bootstrap-editable/bootstrap-editable.css') }}
@endpush
@section('content')
<div class="col-md-12 content-header" >
    <h5><i class="fa fa-globe"></i> Languages Manager</h5>
</div>
    ...
    <div class="col-md-9">
        <div class="card border-top-primary">
            <div class="card-header">
                <a href="{{ route('settings.translation.index') }}" class="btn btn-sm btn-info"><i class="fa fa-backward"></i> Back to Languages</a>
            </div>
            ...
            <div class="card-body table-responsive">
                ...
                {!! Form::close() !!}
                @if($group)
                {!! Form::open(['url' => ['translations/add', $group],'class'=>'mb-3']) !!}
                <div class="form-group ">
                        ...
                        @foreach($translations as $key => $translation)
                            <tr id="{{ $key }}">
                                <td>{{ $key }}</td>
                                    ...
                                    <td>
                                        <a href="#" class="editable status-{{ $t ? $t->status : 0 }} locale-{{ $locale }}" data-locale="{{ $locale }}" data-name="{{ $locale . "|" . $key }}" id="{{ $key }}" data-type="textarea" data-pk="{{ $t ? $t->id : 0 }}" data-url="{{ $editUrl }}" data-title="Enter translation">{{ $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' }}</a>
                                    </td>
                                @if($deleteEnabled)
                                    <td>
                                        <a href="<?= action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key]) ?>" class="delete-key" data-msg="Are you sure you want to delete the translations for '{{ $key }}'?"><span class="fa fa-trash"></span></a>
                                    </td>
                                @endif
                            </tr>
                        @endforeach
